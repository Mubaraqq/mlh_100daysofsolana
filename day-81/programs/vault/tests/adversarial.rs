...
// ==========================================================
// WHAT: A legitimate withdrawal should succeed
// WHY: Proves the defenses don't block honest callers
// ==========================================================
#[test]
fn legitimate_withdraw_succeeds() {
    let (mut svm, _payer) = setup();
    ...
    // This should succeed
    svm.send_transaction(tx).expect("legitimate withdraw should succeed");

    // Read the vault back and check the balance dropped
    let account = svm.get_account(&vault_pda).unwrap();
    let mut data = account.data.as_slice();
    let vault = Vault::try_deserialize(&mut data).unwrap();

    assert_eq!(vault.balance, 300, "balance should be 500 - 200 = 300");
}